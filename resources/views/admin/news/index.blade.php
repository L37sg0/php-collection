@php use App\Models\News;use Illuminate\Support\Str; @endphp
...
@section('content_header')
    ...
    <div
        ...
        <h1 class="h2">{{trans('News')}}</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                <a href="{{ route('admin.news.edit') }}"
                   class="btn btn-sm btn-outline-success">{{trans('New NewsItem')}}</a>
            </div>
        </div>
    </div>
        ...
        <table class="table table-striped table-sm">
            ...
            @foreach($news as $newsItem)
                <tr>
                    ...
                    <td>{{ $newsItem->slug }}</td>
                    <td>{!! Str::limit(strip_tags($newsItem->content), 50) !!}</td>
                    <td>
                        <a href="{{ route('admin.news.edit', ['id' => $newsItem->id]) }}"
